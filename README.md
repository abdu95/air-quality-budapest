...
1. Enable the `openaq_dag` DAG (toggle it on).
...
After a successful `dbt run` you will see `mart_aqi_hourly` materialised as a table in BigQuery.
...
Go to [Looker Studio](https://lookerstudio.google.com) → **Create report** → **Add data source** → **BigQuery** → select `airquality-bp-dezoomcamp` / `air_quality_dataset` / `mart_aqi_hourly`.
