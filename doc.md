...
Dashboard in Looker
    ...
    Build your charts
    Share → "Anyone with the link can view" → copy URL


Chart 1 — Categorical: AQI Category Distribution
What it shows: How many hourly readings fall into each air quality category (Good / Fair / Poor / etc.)
Chart type: Bar chart or Pie chart
Looker Studio config:

Dimension: aqi_category
Metric: COUNT(location_id) (or Record Count)
Sort: by metric descending

Why it works: aqi_category is exactly the categorical variable the rubric asks for. It answers "what proportion of time was the air quality Poor vs Fair?"

Chart 2 — Temporal: AQI Score Over Time
What it shows: How the overall AQI score fluctuates across dates and hours
Chart type: Time series / Line chart
Looker Studio config:

Dimension: measured_date (Looker will handle the time axis automatically)
Breakdown dimension: aqi_category (optional — colors each line segment by category)
Metric: AVG(aqi_score)
Sort: measured_date ascending
